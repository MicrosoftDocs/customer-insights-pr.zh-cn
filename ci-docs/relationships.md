...
3. 在 **新建关系** 窗格中，提供以下信息：
   ...
   - **来源基数**：源实体的基数。 基数描述了集中可能的元素的数量。 它始终与目标基数相关。 您可以在 **一个** 和 **多个** 之间做出选择。 仅支持多对一和一对一关系。  
   ...
   - **目标基数**：目标实体记录的基数。
   - **源键字段**：源实体中的外键字段。 示例：SupportCase 使用 **CaseID** 作为外键字段。
   - **目标键字段**：目标实体的键字段。 示例：客户使用 **CustomerID** 作为键字段。
...
1. 选择 **客户层次结构** 选项卡。
1. 选择 **新建客户层次结构**。
1. 在 **客户层次结构** 窗格中，为层次结构提供名称。 系统会为输出实体创建名称，但您可以进行更改。
1. 选择包含客户层次结构的实体。 它通常位于包含客户的同一实体中。
1. 从所选实体中选择 **客户 UID** 和 **父 UID**。
1. 选择 **保存** 完成客户层次结构。

## <a name="manage-existing-relationships"></a>管理现有关系

转到 **关系** 页面查看所有已创建的关系、关系的源实体、目标实体和基数。

:::image type="content" source="media/relationships-list.png" alt-text="关系页操作栏中的关系和选项列表。":::

使用 **筛选依据** 或 **搜索关系** 选项定位特定关系。 要查看现有关系及其基数的网络图表，选择 [**可视化工具**](#explore-the-relationship-visualizer)。
...
关系路径描述通过源实体和目标实体之间的关系连接的实体。 其使用场景为：要创建的客户细分或度量中包含统一配置文件实体以外的实体，可通过多个选项到达统一配置文件实体。 不同的关系路径可能产生不同的结果。
