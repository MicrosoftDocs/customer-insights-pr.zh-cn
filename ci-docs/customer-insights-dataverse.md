...
管理员可以在创建 Customer Insights 环境时将 Customer Insights 配置为[使用现有 Dataverse 环境](create-environment.md)。 通过提供 Dataverse 环境的 URL，它会附加到他们的新 Customer Insights 环境。 Customer Insights 和 Dataverse 环境必须托管在同一区域。 

如果不希望使用现有 Dataverse 环境，则系统会为租户中的 Customer Insights 数据创建新环境。 

> [!NOTE]
> 如果您的组织已在其租户中使用了 Dataverse，请记住，[Dataverse 环境创建由管理员控制](/power-platform/admin/control-environment-creation)，这一点很重要。例如，如果您使用组织帐户设置新的 Customer Insights 环境，并且管理员已禁止为除管理员之外的每个人创建 Dataverse 试用环境，则您不能创建新的试用环境。
> 
