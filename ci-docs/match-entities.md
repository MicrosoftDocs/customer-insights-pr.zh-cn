...
> [!IMPORTANT]
> 列表中的第一个实体称为主要实体。 主要实体可用作统一配置文件数据集的基础。 所选的其他实体将添加到此实体中。
>
> 重要考虑因素。
>
...
1. 在 **匹配条件** 页上，使用向上和向下箭头按需要的顺序移动实体，或拖放实体。 例如，选择 **Contacts:eCommerce** 作为主要实体，选择 **CustomerLoyalty:Loyalty** 作为第二个实体。
...
1. 在 **添加规则** 窗格中，配置规则的条件。
   ...
   - **选择实体/字段（第一行）**：选择相关实体和属性，以指定客户可能特有的记录属性。 例如，电话号码或电子邮件地址。 避免按活动-类型属性进行匹配。 例如，购买 ID 可能在其他记录类型中找不到匹配项。
...
在大多数情况下，实体匹配会产生具有合并数据的唯一客户配置文件。 若要动态处理极少数情况下的误报和漏报，您可以为匹配规则定义例外。 在处理完匹配规则后应用例外，并避免匹配所有满足例外条件的记录。
...
1. 选择 **自定义类型** 并选择 **下载模板**。 每个匹配选项都需要一个单独的模板。
