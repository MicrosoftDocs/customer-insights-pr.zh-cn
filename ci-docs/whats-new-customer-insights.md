...
2022 年 9 月的更新包括新功能、性能升级和 bug 修复。

### <a name="export-data-to-hubspot"></a>将数据导出到 HubSpot

将统一客户配置文件的客户细分导出到 HubSpot，然后将其用于电子邮件市场营销。

有关更多信息，请参阅[将客户细分导出到 HubSpot](export-hubspot.md)。

### <a name="remove-a-unified-field-or-entity-from-data-unification"></a>从数据统一中删除统一字段或实体

您可以从数据统一过程中删除字段和实体。

有关详细信息，请参阅[删除统一字段](data-unification-update.md#remove-a-unified-field)。

### <a name="manage-unknown-customer-profiles"></a>管理未知客户资料

令人难忘的个性化取决于您的客户数据的丰富性和完整性，而 Customer Insights 可以帮助您实现这些目标。 您可以为除了 ID 之外没有任何其他信息的用户管理客户资料。

有关详细信息，请参阅[使用 Customer Insights 管理未知资料](manage-unknown-profiles.md)。
